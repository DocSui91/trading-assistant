...
def _db_error(label, exc):
    st.session_state["last_db_error"]=f"{label}: {exc}"
...
def db_save_turbos(turbos):
    ...
    try:
        ...
        _db_error("Turbos schreiben",e)
        return False

def supabase_system_check():
    results=[]
    sb=supabase_client()
    if not sb:
        return [("Supabase Client",False,"URL oder Secret fehlt/Client konnte nicht erstellt werden.")]

    results.append(("Supabase Client",True,"Client wurde erstellt."))

    for table in ["assets","turbos","signals"]:
        try:
            sb.table(table).select("*").limit(1).execute()
            results.append((f"{table}: Lesen",True,"OK"))
        except Exception as e:
            results.append((f"{table}: Lesen",False,str(e)))
            continue

        # Harmloser Schreib-/Löschtest mit eindeutigem Testdatensatz.
        try:
            if table=="assets":
                payload={"isin":"SYSTEMCHECK","name":"__SYSTEMCHECK__","symbol":"SYSCHK",
                         "exchange":"","status":"Watchlist","quantity":0.0,"avg_entry":0.0,
                         "sort_order":999999}
                ...
                if ids:
                    ...
                    sb.table(table).delete().eq("isin","SYSTEMCHECK").execute()
            elif table=="turbos":
                payload={"isin":"SYSTEMCHECK","name":"__SYSTEMCHECK__","status":"Watchlist",
                         "underlying":"","sort_order":999999}
                ins=sb.table(table).insert(payload).execute()
                ids=[r.get("id") for r in (ins.data or []) if r.get("id") is not None]
                if ids:
                    sb.table(table).delete().in_("id",ids).execute()
                ...
                payload={"asset_name":"__SYSTEMCHECK__","symbol":"SYSCHK","isin":"SYSTEMCHECK",
                         "status":"Watchlist","signal_date":datetime.now().isoformat(),
                         "entry_price":0.0,"score":0,"short_score":0,"medium_score":0,
                         "risk_score":0,"rsi":0.0,"news_sentiment":0.0,
                         "recommendation":"SYSTEMCHECK","quantity":0.0,"avg_entry":0.0}
                ins=sb.table(table).insert(payload).execute()
                ids=[r.get("id") for r in (ins.data or []) if r.get("id") is not None]
                if ids:
                    sb.table(table).delete().in_("id",ids).execute()
            ...
            results.append((f"{table}: Schreiben/Löschen",True,"OK"))
        except Exception as e:
            results.append((f"{table}: Schreiben/Löschen",False,str(e)))

    return results

def save_current_portfolio():
    if not supabase_client():
        st.session_state["last_db_error"]="Supabase Client nicht verfügbar."
        return False
    ok_assets=db_save_assets(st.session_state.assets)
    ok_turbos=db_save_turbos(st.session_state.turbos)
    return ok_assets and ok_turbos
...
def load_initial_state():
    ...
    if "assets" not in st.session_state:
        st.session_state.assets = db_assets if db_assets is not None else [x.copy() for x in STARTER_ASSETS]

    if "turbos" not in st.session_state:
        st.session_state.turbos = db_turbos if db_turbos is not None else [x.copy() for x in STARTER_TURBOS]
...
st.caption("Version 2.9.2 • Supabase-Systemcheck + transparente Datenbankfehler")

with st.expander("🗄️ Supabase-Systemcheck",expanded=False):
    st.caption("Dieser Test prüft die echte Datenbankverbindung einschließlich Lesen, Schreiben und Löschen.")
    if st.button("▶️ Datenbank jetzt testen",key="run_supabase_check"):
        checks=supabase_system_check()
        ...
        if all_ok:
            st.success("✅ Persistente Speicherung: AKTIV")
        else:
            st.warning("Persistente Speicherung ist noch nicht vollständig funktionsfähig.")

    if st.session_state.get("last_db_error"):
        st.error("Letzter Datenbankfehler: "+st.session_state["last_db_error"])
        ...
        if st.button("➕ Aktie hinzufügen",type="primary",use_container_width=True):
            ...
            if not isin_val and not symbol_val:
                ...
                if existing:
                    ...
                    db_ok=save_current_portfolio()
                    if not db_ok:
                        st.error("Aktie wurde in dieser Sitzung hinzugefügt, aber NICHT dauerhaft in Supabase gespeichert. Öffne den Supabase-Systemcheck für Details.")
...
st.divider()
st.caption("Trading Assistant • Version 2.9.2")
