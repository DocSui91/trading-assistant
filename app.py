...
def autofill_stock_from_isin():
    isin=st.session_state.get("new_isin","")
    if not isin.strip():
        return
...
st.caption("Version 2.9 • ISIN-Autovervollständigung + bearbeitbare Aktien + Formular-Reset")
...
with stocks:
    ...
    st.caption("ISIN eingeben: Wenn dein Twelve-Data-Zugang ISIN-Abfragen erlaubt, werden Name, Ticker und Börse automatisch ergänzt.")
    ...
    isin=st.text_input(
        ...
        on_change=autofill_stock_from_isin
    )
    ...
    with c4:
        quantity=st.number_input(
            "Stückzahl (bei Depot)",
            min_value=0.0,
            value=float(st.session_state.get("new_quantity",0.0)),
            step=1.0,
            key="new_quantity"
        )
    with c5:
        avg_entry=st.number_input(
            ...
            min_value=0.0,
            value=float(st.session_state.get("new_avg_entry",0.0)),
            step=1.0,
            key="new_avg_entry"
        )
        ...
        with st.expander(title,expanded=False):
            ...
            with e1:
                ...
                edit_symbol=st.text_input(
                    "Ticker",
                    value=a.get("symbol",""),
                    key=f"edit_symbol_{i}"
                )
            with e2:
                edit_exchange=st.text_input(
                    "Börse",
                    value=a.get("exchange",""),
                    key=f"edit_exchange_{i}"
                )
...
st.divider()
st.caption("Trading Assistant • Version 2.9")
