...
def german_news_for_asset(asset, limit=8):
    aliases=[a.lower() for a in aliases_for_asset(asset) if len(a)>=3]
    matches=[]
    seen=set()
...
def german_news_for_turbo(turbo, limit=8):
    ...
    if not underlying:
        return []
    fake_asset={"name":underlying,"symbol":underlying.upper(),"isin":""}
    aliases=[underlying.lower()]
    if underlying.upper() in SEARCH_ALIASES:
        aliases += [x.lower() for x in SEARCH_ALIASES[underlying.upper()]]
    matches=[]
    seen=set()
    for feed_cfg in GERMAN_RSS_FEEDS:
        for item in load_rss_feed(feed_cfg["url"]):
            ...
            if any(alias in hay for alias in aliases):
                ...
                seen.add(key)
                matches.append({**item,"source":feed_cfg["name"],"type":feed_cfg["type"]})
    return matches[:limit]


st.title("📈 Trading Assistant")
st.caption("Version 2.6 • dauerhafte Datenbank + Dashboard-Hype-Radar + deutsche News")
...
with st.sidebar:
    ...
    if supabase_client():
        ...
        st.info("Supabase noch nicht eingerichtet – Änderungen sind nicht dauerhaft gespeichert.")
...
st.divider()
st.caption("Trading Assistant • Version 2.6")
